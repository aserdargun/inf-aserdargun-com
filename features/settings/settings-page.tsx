...
export function SettingsPage() {
  ...
  const exportData = () => { if (!health || exporting) return; setExporting(true); setExportError(false); try { downloadInventory(health); } catch { setExportError(true); } finally { setExporting(false); } };
  if (state === "loading") return <div className="settings-page"><header><h1>Settings</h1></header><PageState kind="loading" title="Loading Settings…" /></div>;
  if (state === "error" || !health) return <div className="settings-page"><header><h1>Settings</h1></header><PageState action={<RetryButton onRetry={() => void load()} />} kind="error" title="Settings could not be loaded. Try again." /></div>;
