...
export function SurprisePage() {
  ...
  return <div className="learning-page surprise-page"><PageHeader description="A different infographic for your attention." title="Surprise" />{state === "loading" ? <PageState kind="loading" layout="stage" title="Finding an infographic…" /> : null}{state === "error" ? <PageState action={<RetryButton onRetry={() => void select()} />} kind="error" layout="stage" title="A surprise could not be loaded. Try again." /> : null}{state === "empty" ? <PageState action={<a className="button button--primary" href={routes.library}>Go to Library</a>} kind="empty" layout="stage" title="No active infographics are available." /> : null}{state === "success" && item ? <section aria-label="Selected infographic" className="surprise-selection learning-stage"><MediaCanvas className="learning-media" variant="learning"><Image alt={item.title} fill priority sizes="(max-width: 767px) calc(100vw - 40px), min(900px, 70vw)" src={`/api/public/images/${item.originalDriveFileId}`} /></MediaCanvas><h2>{item.title}</h2><div className="learning-actions"><button className="button button--primary" disabled={selecting} onClick={() => void select()} type="button"><Sparkles aria-hidden="true" size={20} strokeWidth={1.75} />Show another</button><a className="button button--secondary" href={`/infographic/${item.id}`}>Open infographic</a></div></section> : null}</div>;
}
