...
export function InfographicTile({ item }: { item: MaterializedInfographic }) {
  ...
  return <article className="infographic-tile"><a aria-label={`Open ${item.title}`} href={`/infographic/${encodeURIComponent(item.id)}`}><MediaCanvas className="infographic-tile__image" variant="gallery"><ResilientImage alt={item.title} decoding="async" fallbackLabel={`${item.title} preview unavailable`} fallbackText="Preview unavailable" height={TILE_HEIGHT} loading="lazy" src={`/api/public/images/${encodeURIComponent(item.thumbnailDriveFileId)}`} width={TILE_WIDTH} /></MediaCanvas><div className="infographic-tile__caption"><strong>{item.title}</strong><span>{date}</span></div></a></article>;
}
