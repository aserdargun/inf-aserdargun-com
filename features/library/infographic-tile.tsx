...
export function InfographicTile({ item }: { item: MaterializedInfographic }) {
  ...
  return <article className="infographic-tile"><a aria-label={`Open ${item.title}`} href={`/infographic/${encodeURIComponent(item.id)}`}><MediaCanvas className="infographic-tile__image" variant="gallery">{imageFailed ? <span aria-label={`${item.title} preview unavailable`} className="media-canvas__error" role="img">Preview unavailable</span> : <img alt={item.title} decoding="async" height={TILE_HEIGHT} loading="lazy" onError={() => setImageFailed(true)} src={`/api/public/images/${encodeURIComponent(item.thumbnailDriveFileId)}`} width={TILE_WIDTH} />}</MediaCanvas><div className="infographic-tile__caption"><strong>{item.title}</strong><span>{date}</span></div></a></article>;
}
