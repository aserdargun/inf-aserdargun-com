...
import { headersForPath, rewrittenPathFor } from "./static-security-contract.mjs";
...
const server = http.createServer(async (request, response) => {
  if (request.method !== "GET" && request.method !== "HEAD") { response.writeHead(405, { ...headersForPath(staticConfig, "/"), "cache-control": "private, no-store" }).end(); return; }
  let pathname;
  try { pathname = new URL(request.url ?? "/", `http://127.0.0.1:${port}`).pathname; } catch { response.writeHead(400, { ...headersForPath(staticConfig, "/"), "cache-control": "private, no-store" }).end(); return; }
  const file = await fileFor(rewrittenPathFor(staticConfig, pathname));
  if (!file) { response.writeHead(404, { ...headersForPath(staticConfig, pathname), "cache-control": "private, no-store" }).end(); return; }
  const bytes = await readFile(file);
  response.writeHead(200, { ...headersForPath(staticConfig, pathname), "content-type": mime.get(extname(file)) ?? "application/octet-stream", "content-length": String(bytes.length) });
