...
const ownerItem = {
  id: "00000000-0000-4000-8000-000000000201", title: "Systems thinking map", notes: null,
  ...
  seenCount: 0, categoryIds: [], tagIds: [], folderState: "Library", reviewCount: 0, lastReviewedAt: null, reviewDueAt: "2026-08-30T10:00:00.000Z",
};
const publicItem = {
...
async function mockOwnerEvidence(page: Page) {
  ...
  await page.route("**/api/settings/stats", (route) => route.fulfill({ contentType: "application/json", body: JSON.stringify({ total: 1, uncategorized: 1, library: 1, archive: 0, due: 1, reviewed: 0, seen: 0 }) }));
...
async function mockPublicEvidence(page: Page) {
  ...
  await page.route("**/api/public/infographics**", (route) => route.fulfill({ contentType: "application/json", body: JSON.stringify({ items: [publicItem], page: 1, pageSize: 12, totalItems: 1, totalPages: 1 }) }));
  await page.route("**/api/public/images/**", (route) => route.fulfill({ body: image, contentType: "image/png" }));
}
