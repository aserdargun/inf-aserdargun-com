...
const ids = { public: "public-root", private: "private-root", library: "library", thumbnails: "thumbnails", duplicates: "duplicates", events: "events" };
...
class Storage implements StoragePort {
  ...
  async isDescendant(fileId: string, rootId: string): Promise<boolean> { if (this.fail.has(fileId)) throw new Error("refresh_token=never-leak"); return fileId === rootId || (rootId === ids.public && [ids.library, ids.thumbnails, ids.duplicates].includes(fileId)) || (rootId === ids.private && fileId === ids.events); }
}
